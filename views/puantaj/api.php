...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'defter-bazli-rapor') {
    ...
    try {
        ...
        // bitis_tarihi NULL (aktif) VEYA seçilen dönem aralığında aktif olan defterleri getir
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'defter-ozet-rapor') {
    ...
    try {
        ...
        foreach ($donemler as $donem) {
            ...
            // Kalan abone hesapla
            $genel[$donem]['sub_kalan'] = max(0, $genel[$donem]['sub_toplam'] - $genel[$donem]['sub_okunan']);
