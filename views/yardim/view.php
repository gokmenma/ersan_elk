    ...
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mt-4 d-grid gap-2" id="ticket-actions" style="display:none;">
                    <button id="btn-in-progress" class="btn btn-primary w-100 shadow-sm" onclick="updateStatus('isleme_alindi')">
                        <i class="bx bx-loader-circle me-1"></i> İşleme Al
                    </button>
                    <button id="btn-solve-ticket" class="btn btn-success w-100 shadow-sm" onclick="updateStatus('cozuldu')">
            ...
            if (isApprovedTicket) {
                ...
                if (currentViewerIsAdmin) {
                    if (ticket.durum === 'kapali') {
                        $('#btn-in-progress, #btn-solve-ticket, #btn-close-ticket').hide();
                        ...
                        if (ticket.durum === 'isleme_alindi') {
                            ...
                            $('#btn-solve-ticket').show();
                        } else if (ticket.durum === 'cozuldu') {
                            $('#btn-solve-ticket').hide();
                            $('#btn-in-progress').show();
                        } else {
                            $('#btn-in-progress, #btn-solve-ticket').show();
                        }
                    }
                } else {
                    // Kullanıcı sadece kapatabilir
                    $('#btn-in-progress, #btn-solve-ticket, #btn-reopen-ticket').hide();
                    if (ticket.durum === 'kapali') {
...
function updateStatus(status) {
    let actionText = '';
    if (status === 'kapali') actionText = 'kapatmak';
    else if (status === 'acik') actionText = 'açmak';
    else if (status === 'isleme_alindi') actionText = 'işleme almak';
    else if (status === 'cozuldu') actionText = 'çözüldü olarak işaretlemek';

    Swal.fire({
        title: 'Emin misiniz?',
        text: `Bileti ${actionText} istediğinize emin misiniz?`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Evet',
        cancelButtonText: 'Hayır'
