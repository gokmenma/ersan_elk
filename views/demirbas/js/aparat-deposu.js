$(document).ready(function () {
    ...
    $('#sayacPersonelTable').on('click', '.elinde-kalan-indir', function() {
        ...
        form.append('<input type="hidden" name="kategori" value="aparat">');
        $('body').append(form);
        form.submit();
        form.remove();
    });

    // Personele aparat ver butonu
    $('#btnPersonelAta').on('click', function() {
        if ($('#zimmetModal').length) {
            $('#zimmetModal').modal('show');
        } else {
            console.log("Zimmet modal tanımlı değil. (Zimmet Ver Modal'ını aparat-deposu.php'ye eklemelisiniz.)");
        }
    });
