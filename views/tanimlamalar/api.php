...
// Defter Kodu İcmal (Bölge Bazlı Özet - Filtre Uyumlu)
if (isset($_POST["action"]) && $_POST["action"] == "defter-kodu-icmal") {
    try {
        $results = $Tanimlamalar->getFilteredAggregates('defter_kodu', $_POST);

        $html = '<table class="table table-sm table-bordered mb-0">
                    <thead class="bg-light">
                        <tr>
                            <th class="text-center">BÖLGE</th>
                            <th class="text-center">DEFTER</th>
        ...
        foreach ($results as $row) {
            $totalDefter += $row['defter_sayisi'];
            $totalAbone += $row['abone_sayisi'];
            $html .= '<tr>
                        <td><a href="javascript:void(0);" class="icmal-bolge-filter" data-bolge="' . htmlspecialchars($row['bolge']) . '">' . htmlspecialchars($row['bolge'] ?? 'TANIMSIZ') . '</a></td>
                        <td class="text-center">' . number_format($row['defter_sayisi'], 0, ',', '.') . '</td>
                        <td class="text-center">' . number_format($row['abone_sayisi'], 0, ',', '.') . '</td>
                      </tr>';
        }
        ...
        $html .= '</tbody>
                    <tfoot class="bg-light fw-bold">
                        <tr>
                            <td>TOPLAM</td>
                            <td class="text-center">' . number_format($totalDefter, 0, ',', '.') . '</td>
                            <td class="text-center">' . number_format($totalAbone, 0, ',', '.') . '</td>
                        </tr>
                    </tfoot>
                 </table>';
...
// Defter Kodu Excel Aktar (Tüm Filtrelenmiş Kayıtlar)
if (isset($_REQUEST["action"]) && $_REQUEST["action"] == "defter-kodu-excel") {
    try {
        require_once dirname(__DIR__, 2) . '/Autoloader.php';
        $params = $_REQUEST;
        $params['start'] = 0;
        $params['length'] = 50000; // Makul bir üst sınır
        
        $result = $Tanimlamalar->getServerSideData('defter_kodu', $params);
        ...
        // Başlıklar
        $headers = ['ID', 'Defter Kodu', 'Bölgesi', 'Mahalle', 'Abone Sayısı', 'Başlangıç Tarihi', 'Bitiş Tarihi', 'Açıklama'];
        ...
        foreach ($dataRows as $row) {
            $sheet->setCellValue('A' . $rowIdx, $row->id);
            $sheet->setCellValue('B' . $rowIdx, $row->tur_adi);
            $sheet->setCellValue('C' . $rowIdx, $row->defter_bolge);
            $sheet->setCellValue('D' . $rowIdx, $row->defter_mahalle);
            $sheet->setCellValue('E' . $rowIdx, $row->defter_abone_sayisi);
            $sheet->setCellValue('F' . $rowIdx, $row->baslangic_tarihi ? date('d.m.Y', strtotime($row->baslangic_tarihi)) : '');
            $sheet->setCellValue('G' . $rowIdx, $row->bitis_tarihi ? date('d.m.Y', strtotime($row->bitis_tarihi)) : '');
            $sheet->setCellValue('H' . $rowIdx, $row->aciklama);
            $rowIdx++;
        }
        ...
        $filename = 'defter_kodlari_export_' . date('Y-m-d_H-i-s') . '.xlsx';
