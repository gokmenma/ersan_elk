...
- `Ödeme Mutabakatı` sayfasının sırası muhasebe işlem akışını izler: önce açık yeşil zeminde resmî banka matrahına eklenecek kalemler, sonra açık kırmızı zeminde çıkarılacak kesintiler, ardından personelin hesabına yatırılacak banka tutarı ve son olarak açık mavi zeminde bilgi/kontrol alanları gösterilir. Kesinti türleri bilgi ayrıntısıdır; uygulanacak toplamlar `Bankadan Düşülecek` ve `Elden Düşülecek` sütunlarıdır.
