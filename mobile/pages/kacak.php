...
$yetkiIptalGeriAl = Gate::allows('kacak_iptal') || Gate::isSuperAdmin();
    ...
    <div class="px-4 -mt-3 space-y-3">
        ...
        <div class="space-y-3">
            ...
            <div role="button" tabindex="0" data-id="<?= mkH($enc) ?>" data-status="<?= mkH($filterStatus) ?>" data-search="<?= mkH(mb_strtolower(implode(' ', [$kayit['tutanak_no'],$kayit['sayac_no'],$kayit['abone_adi'],$kayit['ekip_adi'],$kayit['ilce']]))) ?>" class="kacak-card w-full text-left bg-white dark:bg-card-dark rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-4 active:scale-[.99] transition-transform">
                ...
                <div class="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 text-[10px] text-slate-400"><span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">speed</span><?= mkH($kayit['sayac_no'] ?: '-') ?></span><span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">photo_camera</span><?= (int)$kayit['foto_sayisi'] ?></span><?php if ((int)$kayit['foto_sayisi'] > 0): ?><a href="../views/kacak/api.php?action=download-zip&amp;token=<?= urlencode($enc) ?>" class="kacak-download ml-auto flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300" title="Fotoğrafları ZIP olarak indir" aria-label="Fotoğrafları ZIP olarak indir"><span class="material-symbols-outlined text-lg">download</span></a><?php endif; ?></div>
            </div>
...
<script>
(() => {
 const details=<?= json_encode($detaylar, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_INVALID_UTF8_SUBSTITUTE) ?: '{}' ?>, csrf=<?= json_encode($mobileCsrf) ?>, canApprove=<?= $yetkiOnay?'true':'false' ?>, canCancel=<?= $yetkiIptalEkle?'true':'false' ?>, canRevertCancel=<?= $yetkiIptalGeriAl?'true':'false' ?>, esc=v=>$('<div>').text(v??'-').html(); let status='', lightboxImages=[], lightboxIndex=0, actionState=null;
 ...
 $('.kacak-card').on('click',function(e){if($(e.target).closest('.kacak-download').length)return;let d=details[this.dataset.id];if(!d)return;let row=(l,v)=>`<div><small class="text-slate-400">${esc(l)}</small><p class="font-semibold break-words">${esc(v)}</p></div>`,gallery=d.foto>0?`<div class="grid grid-cols-[1fr_auto] gap-2 mt-2"><button id="kacakGalleryButton" class="rounded-xl bg-red-600 px-3 py-3 text-white font-bold">Fotoğrafları Görüntüle (${d.foto})</button><a href="../views/kacak/api.php?action=download-zip&token=${encodeURIComponent(d.token)}" class="flex w-12 items-center justify-center rounded-xl bg-emerald-600 text-white" title="Fotoğrafları ZIP olarak indir" aria-label="Fotoğrafları ZIP olarak indir"><span class="material-symbols-outlined">download</span></a></div>`:'',actions='';if(canApprove&&d.onay==='beklemede')actions+='<button id="approveKacak" class="rounded-xl bg-emerald-600 py-3 text-white font-bold">Bildirimi Onayla</button><button id="rejectKacak" class="rounded-xl bg-amber-600 py-3 text-white font-bold">Bildirimi Reddet</button>';if(d.durum==='iptal'&&canRevertCancel)actions+='<button id="revertKacak" class="rounded-xl bg-sky-600 py-3 text-white font-bold">İptali Geri Al</button>';if(d.durum!=='iptal'&&canCancel)actions+='<button id="cancelKacak" class="rounded-xl bg-slate-700 py-3 text-white font-bold">Tutanağı İptal Et</button>';$('#kacakDetailBody').html(`<div class="grid grid-cols-2 gap-3">${row('İşlem durumu',d.durum)}${row('Onay durumu',d.onay)}${row('Tarih',d.tarih)}${row('İlçe',d.ilce)}${row('Tür',d.tur)}${row('Tutanak no',d.tutanak)}${row('Abone',d.abone)}${row('Sayaç no',d.sayac)}${row('Endeks',d.endeks)}${row('Fotoğraf',d.foto)}</div>${row('Ekip / Bildiren',d.ekip)}${row('Açıklama',d.aciklama)}${gallery}<div class="grid gap-2 pt-2">${actions}</div>`);$('#kacakGalleryButton').on('click',()=>openKacakGallery(d.token));$('#approveKacak').on('click',()=>kacakAction('approve',d.token));$('#rejectKacak').on('click',()=>kacakPrompt('reject',d.token));$('#cancelKacak').on('click',()=>kacakPrompt('cancel',d.token));$('#revertKacak').on('click',()=>kacakAction('revert-cancel',d.token));$('#kacakOverlay').removeClass('hidden');requestAnimationFrame(()=>$('#kacakSheet').removeClass('translate-y-full'))});
