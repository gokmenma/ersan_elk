...
foreach ($ihbarlar as $ihbar) {
    ...
    $bildirenAdi = $ihbar->bildiren_personel_adi ?: ($ihbar->olusturan_user_adi ?: 'Belirtilmedi');
    $detaylar[$anahtar] = [
...
foreach ($ihbarlar as $ihbar) {
    ...
    }, preg_split('/[,&]/', (string) ($ihbar->atanan_ekip_adi ?? '')))));
    ...
    <div class="px-4 -mt-3 space-y-3">
        ...
        <div id="ihbarList" class="space-y-3">
            <?php foreach ($ihbarlar as $ihbar): 
                $d = $durumlar[$ihbar->durum] ?? [$ihbar->durum, 'bg-slate-100 text-slate-600']; 
                $enc = $sifreliIhbarIdleri[(int) $ihbar->id]; 
                $bildirenAdi = $ihbar->bildiren_personel_adi ?: ($ihbar->olusturan_user_adi ?: '');
            ?>
            <button type="button" data-status="<?= miH($ihbar->durum) ?>" data-search="<?= miH(mb_strtolower(($ihbar->ilce ?? '').' '.($ihbar->mahalle ?? '').' '.($ihbar->aciklama ?? '').' '.($ihbar->atanan_ekip_adi ?? '').' '.$bildirenAdi)) ?>" data-id="<?= miH($enc) ?>" class="ihbar-card w-full text-left bg-white dark:bg-card-dark rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm p-4 active:scale-[.99] transition-transform">
                <div class="flex justify-between gap-3">
                    <div class="min-w-0">
                        <h2 class="font-bold text-slate-800 dark:text-white truncate"><?= miH(($ihbar->ilce ?: 'İlçe belirtilmedi') . ($ihbar->mahalle ? ' · '.$ihbar->mahalle : '')) ?></h2>
                        <p class="text-xs text-slate-500 mt-1 line-clamp-2"><?= miH($ihbar->aciklama ?: '-') ?></p>
                    </div>
                    <span class="shrink-0 h-fit px-2.5 py-1 rounded-full text-[10px] font-bold <?= miH($d[1]) ?>"><?= miH($d[0]) ?></span>
                </div>
                <div class="flex flex-col gap-1 mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 text-[10px] text-slate-400">
                    <div class="flex items-center justify-between gap-2">
                        <span class="flex items-center gap-1 text-slate-600 dark:text-slate-300 font-medium truncate">
                            <span class="material-symbols-outlined text-sm shrink-0 text-slate-400">person</span>
                            <span class="truncate"><?= miH($bildirenAdi ?: 'Belirtilmedi') ?></span>
                        </span>
                        <span class="shrink-0 text-slate-400"><?= miH(date('d.m.Y H:i', strtotime($ihbar->created_at))) ?></span>
                    </div>
                    <div class="flex items-center gap-1 text-slate-400">
                        <span class="material-symbols-outlined text-sm shrink-0">groups</span>
                        <span class="truncate"><?= miH($ihbar->atanan_ekip_adi ?: 'Atama bekliyor') ?></span>
                    </div>
                </div>
            </button>
            <?php endforeach; ?>
...
<div id="ihbarSummarySheet" class="fixed bottom-0 inset-x-0 z-[73] translate-y-full transition-transform rounded-t-[2rem] overflow-hidden bg-white dark:bg-card-dark max-h-[85vh] flex flex-col safe-area-bottom shadow-2xl">
    <div class="sticky top-0 bg-white dark:bg-card-dark p-4 border-b border-slate-100 dark:border-slate-800 shrink-0 z-10 rounded-t-[2rem]">
...
<div id="ihbarSheet" class="fixed bottom-0 left-0 right-0 z-[71] translate-y-full transition-transform bg-white dark:bg-card-dark rounded-t-[2rem] overflow-hidden max-h-[82vh] overflow-y-auto safe-area-bottom shadow-2xl">
    <div class="sticky top-0 bg-white dark:bg-card-dark px-5 pt-3 pb-3 border-b border-slate-100 dark:border-slate-800 rounded-t-[2rem]"><div class="w-10 h-1 bg-slate-300 rounded-full mx-auto mb-3"></div><div class="flex justify-between"><h3 class="font-bold text-lg">İhbar Detayı</h3><button onclick="closeIhbarDetail()"><span class="material-symbols-outlined">close</span></button></div></div>
...
<div id="ihbarAssignSheet" class="fixed bottom-0 inset-x-0 z-[81] translate-y-full transition-transform rounded-t-[2rem] overflow-hidden bg-white dark:bg-card-dark max-h-[85vh] flex flex-col safe-area-bottom shadow-2xl">
 <div class="p-4 border-b dark:border-slate-800 bg-white dark:bg-card-dark rounded-t-[2rem]"><div class="w-10 h-1 bg-slate-300 rounded-full mx-auto mb-3"></div><div class="flex items-center justify-between"><div><h2 class="font-bold text-lg">Kaçak Ekibini Seçin</h2><p class="text-xs text-slate-400">En fazla iki personel seçebilirsiniz</p></div><button onclick="closeAssignSheet()" class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800"><span class="material-symbols-outlined">close</span></button></div><div class="relative mt-3"><span class="material-symbols-outlined absolute left-3 top-2.5 text-slate-400">search</span><input id="assignPersonSearch" type="search" placeholder="Personel ara..." class="w-full rounded-xl border-0 bg-slate-100 dark:bg-slate-800 pl-10 text-sm"></div></div>
