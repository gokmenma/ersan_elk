            ...
            <?php foreach ($avanslar as $avans): ?>
                <div class="bg-white dark:bg-card-dark rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-4">
                    ...
                    <div class="flex gap-2">
                        <button onclick="openModal('avansRed', <?= $avans->id ?>, '<?= htmlspecialchars(addslashes($avans->adi_soyadi)) ?>')" class="flex-1 px-3 py-2 bg-red-50 hover:bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5">
                            <span class="material-symbols-outlined text-[16px]">close</span> Reddet
                        </button>
                        <button onclick="openModal('avansOnay', <?= $avans->id ?>, '<?= htmlspecialchars(addslashes($avans->adi_soyadi)) ?>', '<?= formatMoneyMobile($avans->tutar) ?>')" class="flex-1 px-3 py-2 bg-green-500 hover:bg-green-600 text-white rounded-xl text-xs font-bold shadow-sm shadow-green-500/30 transition-colors flex items-center justify-center gap-1.5">
                            <span class="material-symbols-outlined text-[16px]">check</span> Onayla
                        </button>
                    </div>
                    <?php else: ?>
                    <div class="flex justify-end border-t border-slate-100 dark:border-slate-800/60 pt-3">
                        <?php if ($avans->durum == 'onaylandi'): ?>
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400 rounded-lg text-xs font-bold"><span class="material-symbols-outlined text-[16px]">check_circle</span> Onaylandı</span>
                        <?php else: ?>
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-lg text-xs font-bold"><span class="material-symbols-outlined text-[16px]">cancel</span> Reddedildi</span>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
                </div>
        ...
        <?php else: ?>
            ...
            ?>
                <div class="bg-white dark:bg-card-dark rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-4">
                    ...
                    <?php if (!$showApproved): ?>
                    <div class="flex gap-2">
                        <button onclick="openModal('izinRed', <?= $izin->id ?>, '<?= htmlspecialchars(addslashes($izin->adi_soyadi)) ?>')" class="flex-1 px-3 py-2 bg-red-50 hover:bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5">
                            <span class="material-symbols-outlined text-[16px]">close</span> Reddet
                        </button>
                        <button onclick="openModal('izinOnay', <?= $izin->id ?>, '<?= htmlspecialchars(addslashes($izin->adi_soyadi)) ?>', '<?= $izinTuruLabel ?>', <?= $gunSayisi ?>)" class="flex-1 px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-xs font-bold shadow-sm shadow-blue-500/30 transition-colors flex items-center justify-center gap-1.5">
                            <span class="material-symbols-outlined text-[16px]">check</span> Onayla
                        </button>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/60 pt-3 <?= !$showApproved ? 'mb-3' : '' ?>">
                        ...
                        <?php else: ?>
                            <?php if ($talep->durum == 'cozuldu'): ?>
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400 rounded-lg text-xs font-bold"><span class="material-symbols-outlined text-[16px]">check_circle</span> Çözüldü</span>
                            <?php elseif ($talep->durum == 'iptal_edildi'): ?>
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-lg text-xs font-bold"><span class="material-symbols-outlined text-[16px]">cancel</span> İptal Edildi</span>
                            <?php else: ?>
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400 rounded-lg text-xs font-bold"><span class="material-symbols-outlined text-[16px]">info</span> <?= ucfirst($talep->durum) ?></span>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>
