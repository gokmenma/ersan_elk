...
class BordroPersonelModel extends Model
{
    ...
    // Maaşa dahil yardım dağılımında kesintinin kayıtlı ödeme kanalı korunur:
    // banka kesintisi önce banka matrahından, elden kesintisi önce elden
    // hakedişten düşer; ilgili kanal yetmezse kalan tutar diğerine aktarılır.
    private function hesaplaDahilBankaDagilimi(float $toplamHakedis, float $bankaMatrahi, float $bankaKesintisi, float $eldenKesintisi = 0.0): array
    {
        ...
        $dusulenEldenElden = min($eldenBrut, $eldenKesintisi);
        $kalanEldenKesintisi = max(0.0, $eldenKesintisi - $dusulenEldenElden);
        $kalanElden = max(0.0, $eldenBrut - $dusulenEldenElden);

        $dusulenBankaBanka = min($bankaMatrahi, $bankaKesintisi);
        $kalanBankaKesintisi = max(0.0, $bankaKesintisi - $dusulenBankaBanka);
        $kalanBanka = max(0.0, $bankaMatrahi - $dusulenBankaBanka);

        $dusulenEldenBanka = min($kalanBanka, $kalanEldenKesintisi);
        $kalanBanka = max(0.0, $kalanBanka - $dusulenEldenBanka);

        $dusulenBankaElden = min($kalanElden, $kalanBankaKesintisi);
        $kalanElden = max(0.0, $kalanElden - $dusulenBankaElden);

        return [
            'banka' => round($kalanBanka, 2),
            'elden' => round($kalanElden, 2),
            'banka_kesintisi' => round($dusulenBankaBanka + $dusulenEldenBanka, 2),
            'elden_kesintisi' => round($dusulenEldenElden + $dusulenBankaElden, 2),
        ];
    }
    ...
    {
        $aktifGunler = $this->getCalismaGecmisiAktifTarihleri($personelId, $donemBaslangic, $donemBitis);
    ...
    {
        return count($this->birlesikTarihAraligiGunleri($araliklar, $donemBaslangic, $donemBitis));
    }

    private function birlesikTarihAraligiGunleri(array $araliklar, string $donemBaslangic, string $donemBitis): array
    {
        $aktifGunler = [];
    ...
    private function getKarisikMaasGecmisiOzeti(int $personelId, string $donemBaslangic, string $donemBitis): ?array
    {
        ...
        if (!$hasPrim || !$hasSabit) return null;

        $aktifGunler = $this->getCalismaGecmisiAktifTarihleri($personelId, $donemBaslangic, $donemBitis);
        $ucretsizGunler = $this->getUcretsizIzinTarihleri($personelId, $donemBaslangic, $donemBitis);
        $sabitHakedis = 0.0;
        $sabitGun = 0;
        $atananGunler = [];
        ...
        foreach ($kayitlar as $kayit) {
            ...
            for ($ts = $basTs; $ts <= $bitTs; $ts = strtotime('+1 day', $ts)) {
                $tarih = date('Y-m-d', $ts);
    ...
    private function getYemekYardimiGunlukLimitForDate(object $kayit, ?string $parametreTarihi = null): float
    {
        ...
        if (!empty($kayit->yemek_yardimi_parametre_id)) {
            ...
            if ($paramYemek) {
                ...
                $paramTarihGecerli = empty($parametreTarihi)
    ...
    private function hesaplaMaasaDahilYardimDagilimi(object $kayit, float $asgariUcretNet, int $maasHesapGunu, int $fiiliGunSayisi, float $puantajToplami = 0.0, float $toplamKesinti = 0.0, float $bankayaTasinabilirEkOdeme = 0.0, float $sozlesmeHakedisi = 0.0, float $resmiDahilEkToplam = 0.0, float $htcNetFazla = 0.0): array
    {
        ...
        // Asgari Ücretin Dönemlik (Pro-rated) Neti + Resmi alacağa dahil ek ödemelerin toplamı
        $sonuc['asgari_hakedis'] = round(($asgariUcretNet / 30) * $maasHesapGunu + $resmiDahilEkToplam, 2);
        ...
        // Maaşa dahil yardım, sözleşme netini asgari ücretten sonra tamamlayan bakiyedir.
        // Kesintiler yemek kapasitesini düşürmez; ödeme dağıtımında banka matrahından mahsup edilir.
        ...
        // Eş yardımı (Eğer dahilse ve hala yer varsa)
    ...
    /**
     * RTÇ (Resmi Tatil Çalışması) ve HTÇ (Hafta Tatili Çalışması) için SGK işçi payı +
     * İşsizlik işçi payı + Gelir Vergisi (kümülatif dilime göre) + Damga Vergisi kesintilerini
     * uygular. RTÇ her zaman asgari ücretin günlük NET tutarını garanti eden bir hedeftir
     * (brüte tamamlanır/gross-up). HTÇ ise "Maaşa Dahil" personelde aynı şekilde asgari net
     * hedeflidir; "Maaşa Dahil" olmayan personelde ise zaten çalışanın kendi brüt maaş oranına
     * dayalı gerçek bir brüt tutardır (gross-up YAPILMAZ, doğrudan üzerinden kesinti uygulanır).
     *
     * hesaplaMaas() ve hesaplaOrtakGosterimDegerleri() tarafından ortak kullanılır ki bu iki
     * hesaplama yolu birbirinden sapmasın.
     */
    private function hesaplaRtcHtcKesinti(
        float $rtcHedefNet,
        float $htcTaban,
        bool $htcNetHedefMi,
        bool $rtcSgkDahil,
        ...
        $pm = $this->cachedParametreModel;
        $bos = ['brut' => 0.0, 'sgk' => 0.0, 'issizlik' => 0.0, 'gelir_vergisi' => 0.0, 'damga' => 0.0, 'matrah' => 0.0, 'net' => 0.0];

        $rtcSonuc = $rtcHedefNet > 0
            ? $pm->bruteUpForNetTarget(
                $rtcHedefNet,
                $kumulatifMatrahOncesi,
                $rtcSgkDahil ? $sgkOrani : 0.0,
                $rtcSgkDahil ? $issizlikOrani : 0.0,
                $rtcDamgaDahil ? $damgaOrani : 0.0,
                $donemYil,
                $rtcGvDahil
            )
            : $bos;

        // HTÇ'nin marjinal dilim payı RTÇ'den sonra izole edilsin diye kümülatif tabana
        // RTÇ'nin (gv dahilse) matrah katkısı da eklenir.
        $kumulatifTabanHtc = $kumulatifMatrahOncesi + ($rtcGvDahil ? $rtcSonuc['matrah'] : 0.0);

        if ($htcTaban <= 0) {
            $htcSonuc = $bos;
        } elseif ($htcNetHedefMi) {
            $htcSonuc = $pm->bruteUpForNetTarget(
                $htcTaban,
                $kumulatifTabanHtc,
                $htcSgkDahil ? $sgkOrani : 0.0,
                $htcSgkDahil ? $issizlikOrani : 0.0,
                $htcDamgaDahil ? $damgaOrani : 0.0,
                $donemYil,
                $htcGvDahil
            );
        } else {
            // Zaten brüt (çalışanın kendi maaş oranına dayalı HTÇ): doğrudan kesinti uygula
            $sgk = $htcSgkDahil ? round($htcTaban * $sgkOrani, 2) : 0.0;
            $issizlik = $htcSgkDahil ? round($htcTaban * $issizlikOrani, 2) : 0.0;
            $matrah = $htcGvDahil ? max(0, round($htcTaban - $sgk - $issizlik, 2)) : 0.0;
            $gv = $htcGvDahil ? $pm->hesaplaGelirVergisi($kumulatifTabanHtc + $matrah, $matrah, $donemYil) : 0.0;
            $damga = $htcDamgaDahil ? round($htcTaban * $damgaOrani, 2) : 0.0;
            $net = round($htcTaban - $sgk - $issizlik - $gv - $damga, 2);
            $htcSonuc = ['brut' => $htcTaban, 'sgk' => $sgk, 'issizlik' => $issizlik, 'gelir_vergisi' => $gv, 'damga' => $damga, 'matrah' => $matrah, 'net' => $net];
        }

        return [
            'rtc' => $rtcSonuc,
            'htc' => $htcSonuc,
            'toplam_brut' => round($rtcSonuc['brut'] + $htcSonuc['brut'], 2),
            'toplam_net' => round($rtcSonuc['net'] + $htcSonuc['net'], 2),
            'toplam_sgk' => round($rtcSonuc['sgk'] + $htcSonuc['sgk'], 2),
            'toplam_issizlik' => round($rtcSonuc['issizlik'] + $htcSonuc['issizlik'], 2),
            'toplam_gv' => round($rtcSonuc['gelir_vergisi'] + $htcSonuc['gelir_vergisi'], 2),
            'toplam_damga' => round($rtcSonuc['damga'] + $htcSonuc['damga'], 2),
            'toplam_matrah_katkisi' => round(($rtcGvDahil ? $rtcSonuc['matrah'] : 0.0) + ($htcGvDahil ? $htcSonuc['matrah'] : 0.0), 2),
        ];
    }
    ...
    public function getSgkFirmaDagilimi($personel_id, $baslangic, $bitis, $defaultFirma = 'Yok', $iseGiris = null, $istenCikis = null, array $eksikGunler = [])
    {
        ...
        while ($cur !== false && $cur <= $end) {
            ...
            // Çalışma geçmişi yoksa ana personel tarihleri içinde varsayılan firmayı kullan.
            // Geçmiş varsa kayıtsız günleri (özellikle işe giriş öncesini) firmaya yazma.
            if ($activeCompany === null && empty($segments)) {
                $girisUygun = !$this->isValidDateValue($iseGiris) || $tarih >= $iseGiris;
                $cikisUygun = !$this->isValidDateValue($istenCikis) || $tarih <= $istenCikis;
                if ($girisUygun && $cikisUygun) {
            ...
            if (stripos((string)$activeCompany, 'KUR') !== false) {
                $kurDays++;
                if (!isset($eksikGunler[$tarih])) $payableKurDays++;
            } else {
                $nonKurDays++;
                if (!isset($eksikGunler[$tarih])) $payableNonKurDays++;
            }
        ...
        return [
            ...
            'payable_kur_days' => $payableKurDays,
            'payable_non_kur_days' => $payableNonKurDays,
            'total_days' => $totalDays,
            'non_kur_ratio' => $totalDays > 0 ? ($nonKurDays / $totalDays) : 0.0
        ];
    }
    ...
    public function hesaplaOrtakGosterimDegerleri(object $p, ?object $donemBilgi, float $asgariUcretNet): array
    {
        ...
        $karisikMaasOzeti = $this->getKarisikMaasGecmisiOzeti((int) $p->personel_id, $donemBaslangic, $donemBitis);
        ...
        if (isset($p->hesaplama_detay) && !empty($p->hesaplama_detay)) {
            ...
            $icraKesintisi = floatval($detay['odeme_dagilimi']['icra_kesintisi'] ?? 0);
        }
        ...
        // Gösterimde eski hesaplama_detay değerine güvenme; izin ve çalışma geçmişi
        // sonradan değişmiş olabilir. Güncel kayıtlardan tarih bazında yeniden hesapla.
        ...
        if ($hesaplamayaEsasMaas <= 0) {
            ...
            $asgariUcretBrutVal = floatval($this->cachedParametreModel->getGenelAyar('asgari_ucret_brut', $donemBaslangic) ?? 33030.00);
            if ($isNet) {
                $hesaplamayaEsasMaas = floatval($asgariUcretNet);
                $maasTutari = floatval($asgariUcretNet);
            } elseif ($isBrut) {
                $hesaplamayaEsasMaas = $asgariUcretBrutVal;
                $maasTutari = $asgariUcretBrutVal;
            }
        }
        ...
        // Net ücretli maaşa-dahil personelde puantaj, toplam alacağın parçasıdır;
        ...
        foreach ($ekOdemelerList as $eo) {
            ...
            $isYemekOdeme = strpos($eoTurLower, 'yemek') !== false;
        ...
        // RTÇ/HTÇ puantaj günlerinden otomatik bordro etkisi
        $ozelCache = $this->ozelCalismaCache[$p->personel_id] ?? null;
        if ($ozelCache !== null) {
        ...
        // RTÇ/HTÇ: RTÇ her zaman, HTÇ ise "Maaşa Dahil" personelde asgari ücretin günlük net tutarını
        // garanti eden bir hedeftir (brüte tamamlanır/gross-up). "Maaşa Dahil" olmayan personelde HTÇ
        // zaten çalışanın kendi brüt maaş oranına dayalı gerçek bir brüt tutardır (gross-up yapılmaz).
        ...
        if ($rtcHtcBrutGosterimToplam > 0) {
            ...
            $rtcHtcSonucGosterim = $this->hesaplaRtcHtcKesinti(
                ...
                $isInclusive,
                $rtcSgkDahilGosterim, $rtcGvDahilGosterim, $rtcDamgaDahilGosterim,
                $htcSgkDahilGosterim, $htcGvDahilGosterim, $htcDamgaDahilGosterim,
                $sgkOraniGosterim, $issizlikOraniGosterim, $damgaOraniGosterim,
                $kumulatifMatrahGosterim + $anaMaasMatrahGosterim,
                $donemYilGosterim
            );

            $rtcHtcMatrahKatkisiGosterim = $rtcHtcSonucGosterim['toplam_matrah_katkisi'];
            $rtcHtcNetOdemeGosterim = $rtcHtcSonucGosterim['toplam_net'];
            $rtcHtcKesintiToplamGosterim = round($rtcHtcSonucGosterim['toplam_brut'] - $rtcHtcNetOdemeGosterim, 2);

            $rtcPayGosterim = $rtcHtcSonucGosterim['rtc']['net'];
            if ($rtcPayGosterim > 0) {
                $yontemliBankaEki += $rtcPayGosterim;
                $rtcHtcBankaNetiGosterim += $rtcPayGosterim;
                $bankaEkOdemeDetaylari[] = [
                    'etiket' => 'Resmi Tatil Çalışması (Net)',
                    'tutar' => $rtcPayGosterim
                ];
            }
            $htcPayGosterim = $rtcHtcSonucGosterim['htc']['net'];
            if ($htcPayGosterim > 0) {
                $yontemliBankaEki += $htcPayGosterim;
                $rtcHtcBankaNetiGosterim += $htcPayGosterim;
                $bankaEkOdemeDetaylari[] = [
        ...
        if ($karisikMaasOzeti !== null && !$isInclusive) {
            ...
            $sozlesmeHakedisi = $karisikMaasOzeti !== null
                ? floatval($karisikMaasOzeti['sabit_hakedis'])
                : round(($hesaplamayaEsasMaas / 30) * $calismaGunu, 2);
        ...
        $eksikGunTarihleri = $this->getMaasEksikGunTarihleri((int) $p->personel_id, $donemBaslangic, $donemBitis);
        $dagilim = $this->getSgkFirmaDagilimi(
            $p->personel_id,
            $donemBaslangic,
            $donemBitis,
            $p->sgk_yapilan_firma ?? 'Yok',
            $p->ise_giris_tarihi ?? null,
            $p->isten_cikis_tarihi ?? null,
            $eksikGunTarihleri
        );
        $nonKurRatio = $dagilim['non_kur_ratio'];
        $nonKurUcretGunu = min($calismaGunu, intval($dagilim['payable_non_kur_days'] ?? round($calismaGunu * $nonKurRatio)));
        ...
        if ($manualDagitimVar) {
            ...
            $asgariYatacak = round(($asgariUcretNet / 30) * $nonKurUcretGunu, 2);
            ...
            $digerOdeme = floatval($p->diger_odeme ?? 0);
            $asgariUcretYatacak = round(($asgariUcretNet / 30) * $nonKurUcretGunu, 2);
        ...
        return [
            ...
            'karisikMaasGecmisi' => $karisikMaasOzeti !== null,
    ...
    public function getPersonellerByDonem($donem_id, $ids = [])
    {
        ...
        if ($this->ozelCalismaCache === null) {
            $this->ozelCalismaCache = [];
            $ocQuery = $this->db->prepare("
                SELECT pi.personel_id, pi.baslangic_tarihi, pi.bitis_tarihi,
                       t.resmi_tatil_calismasi, t.hafta_tatili_calismasi
                FROM personel_izinleri pi
                JOIN tanimlamalar t ON t.id = pi.izin_tipi_id
                WHERE pi.onay_durumu = 'Onaylandı'
                AND pi.silinme_tarihi IS NULL
                AND (t.resmi_tatil_calismasi = 1 OR t.hafta_tatili_calismasi = 1)
                AND pi.baslangic_tarihi > ''
                AND pi.bitis_tarihi > ''
                AND pi.baslangic_tarihi <= ?
                AND pi.bitis_tarihi >= ?
            ");
            $ocQuery->execute([$donemBitis, $donemBaslangic]);
            $donemBasDt = new \DateTime($donemBaslangic);
            $donemBitDt = new \DateTime($donemBitis);
            foreach ($ocQuery->fetchAll(PDO::FETCH_OBJ) as $row) {
                $pid = $row->personel_id;
                if (!isset($this->ozelCalismaCache[$pid])) {
                    $this->ozelCalismaCache[$pid] = ['rtc' => 0, 'htc' => 0];
                }
                try {
                    $bas = new \DateTime($row->baslangic_tarihi);
                    $bit = new \DateTime($row->bitis_tarihi);
                    $kesisimBas = max($bas, $donemBasDt);
                    $kesisimBit = min($bit, $donemBitDt);
                    if ($kesisimBas <= $kesisimBit) {
                        $gun = $kesisimBas->diff($kesisimBit)->days + 1;
                        if ($row->resmi_tatil_calismasi) $this->ozelCalismaCache[$pid]['rtc'] += $gun;
                        if ($row->hafta_tatili_calismasi) $this->ozelCalismaCache[$pid]['htc'] += $gun;
                    }
                } catch (\Exception $e) {}
            }
    ...
    public function addPersonellerToDonem($donem_id, $baslangic_tarihi, $bitis_tarihi)
    {
        ...
        AND (
            ...
            OR (p.aktif_mi = 0 AND (p.isten_cikis_tarihi IS NULL OR p.isten_cikis_tarihi = '0000-00-00'))
        )
        AND NOT EXISTS (
    ...
    ");
        ...
        // İşten çıkış tarihi: NULL, '0000-00-00' veya dönem başlangıcından büyük/eşit olanlar
        $sql = $this->db->prepare("
        ...
        AND (
            (
                (ise_giris_tarihi IS NULL OR ise_giris_tarihi = '0000-00-00' OR ise_giris_tarihi <= :bitis_tarihi)
                AND (
                    (aktif_mi = 1 AND (isten_cikis_tarihi IS NULL OR isten_cikis_tarihi = '0000-00-00' OR isten_cikis_tarihi >= :baslangic_tarihi))
                    OR
                    (aktif_mi = 0 AND isten_cikis_tarihi IS NOT NULL AND isten_cikis_tarihi != '0000-00-00' AND isten_cikis_tarihi >= :baslangic_tarihi)
                )
            )
            OR EXISTS (
                SELECT 1 FROM personel_calisma_gecmisi pcg
    ...
    public function olusturNobetOdemeleri($personel_id, $donem_id, $baslangic_tarihi, $bitis_tarihi)
    {
        ...
        foreach ($nobetGruplari as $nobet) {
            ...
            // Hafta İçi (standart) nöbet: kişinin kendi eline geçen tutar parametredeki (varsayılan_tutar)
            // sabit ücrettir; asıl ödenen tutarı ARTIRMAZ. "resmi_tutar" (banka/resmi alacağa dahil edilen
            // kısım) ise asgari ücretin saatlik NET tutarının %50 fazlasının (fazla mesai) SGK işçi payı +
            // İşsizlik işçi payı + Gelir Vergisi (dilime göre) + Damga Vergisi kesintilerinden SONRA tam
            // olarak bu net hedefe ulaşacak şekilde brüte tamamlanmasıyla (gross-up) hesaplanır.
            $resmiTutar = 0.0;
            if ($tip === 'standart') {
                if ($this->cachedParametreModel === null) {
                    $this->cachedParametreModel = new BordroParametreModel();
                }
                $asgariUcretNetNobet = floatval($this->cachedParametreModel->getGenelAyar('asgari_ucret_net', $baslangic_tarihi) ?? 28075.50);
                $sgkOraniNobet = floatval($this->cachedParametreModel->getGenelAyar('sgk_isci_orani', $baslangic_tarihi) ?? 14) / 100;
                $issizlikOraniNobet = floatval($this->cachedParametreModel->getGenelAyar('issizlik_isci_orani', $baslangic_tarihi) ?? 1) / 100;
                ...
                $nobetSgkDahil = !empty($haftaIciParam->sgk_matrahi_dahil ?? 0);
                $nobetGvDahil = !empty($haftaIciParam->gelir_vergisi_dahil ?? 1);
                $nobetDamgaDahil = !empty($haftaIciParam->damga_vergisi_dahil ?? 1);

                $saatlikNetHedef = ($asgariUcretNetNobet / 225) * 1.5;
                $toplamNetHedef = round($saatlikNetHedef * 4 * $adet, 2);
                $grossUp = $this->cachedParametreModel->bruteUpForNetTarget(
                    $toplamNetHedef,
                    $kumulatifMatrahNobet,
                    $nobetSgkDahil ? $sgkOraniNobet : 0.0,
                    $nobetSgkDahil ? $issizlikOraniNobet : 0.0,
                    $nobetDamgaDahil ? $damgaOraniNobet : 0.0,
                    $donemYilNobet,
                    $nobetGvDahil
                );
                $resmiTutar = $grossUp['brut'];
            }
     ...
     * @param int $maasHesapGunu (Opsiyonel: Maaşa dahil dengeleme için çalışılan gün sayısı)
     */
    public function olusturProfilBazliOdemeler($personel_id, $donem_id, $baslangic_tarihi, $maasHesapGunu = 30)
    {
        $PersonelModel = new \App\Model\PersonelModel();
        $personel = $PersonelModel->find($personel_id);
        ...
        if (!empty($personel->yemek_yardimi_aliyor) && !empty($personel->yemek_yardimi_parametre_id)) {
            ...
            if ($param) {
                ...
                if ($yemekYardimiDahil === 1) {
                    // Maaşa dahilse dengeleme tutarını hesapla
    ...
    public function olusturIcraKesintileri($personel_id, $donem_id, $baslangic_tarihi, $bitis_tarihi)
    {
        ...
        ");
        $sql->execute([$personel_id, $bitis_tarihi, $baslangic_tarihi]);
        $processQueue = $sql->fetchAll(PDO::FETCH_OBJ);
        ...
        while ($idx < count($processQueue)) {
            ...
            if ($kalanBorc <= 0) {
                ...
                $this->db->prepare("UPDATE personel_icralari SET durum = 'kesinti_bitti', updated_at = NOW() WHERE id = ?")
                    ->execute([$icra->id]);

                $nextIcra = $this->baslaSiradakiBekleyenIcra($personel_id);
                if ($nextIcra) {
                    $processQueue[] = $nextIcra;
                }
                continue;
            }
    ...
    private function baslaSiradakiBekleyenIcra($personel_id)
    {
        $sqlNext = $this->db->prepare("
            SELECT id, icra_dairesi, dosya_no, aylik_kesinti_tutari, baslangic_tarihi, toplam_borc, sira, kesinti_tipi, kesinti_orani
            FROM personel_icralari
            WHERE personel_id = ?
            AND durum = 'bekliyor'
            ...
            LIMIT 1
        ");
        $sqlNext->execute([$personel_id]);
        $nextIcra = $sqlNext->fetch(PDO::FETCH_OBJ);

        if ($nextIcra) {
            $this->db->prepare("UPDATE personel_icralari SET durum = 'devam_ediyor', updated_at = NOW() WHERE id = ?")
                ->execute([$nextIcra->id]);
        }

        return $nextIcra ?: null;
    }
    ...
    public function getUcretliIzinGunu($personel_id, $donem_baslangic, $donem_bitis)
    {
        ...
        if ($p) {
            if ($p && !empty($p->ise_giris_tarihi) && $p->ise_giris_tarihi !== '0000-00-00') {
                if ($p->ise_giris_tarihi > $aktifBaslangic) {
                    $aktifBaslangic = $p->ise_giris_tarihi;
                }
            }
            if ($p && !empty($p->isten_cikis_tarihi) && $p->isten_cikis_tarihi !== '0000-00-00') {
    ...
    public function getOzelCalismaGunSayisi(int $personel_id, string $donem_baslangic, string $donem_bitis, string $flagKolonu): int
    {
        $sql = $this->db->prepare("
            ...
            AND pi.bitis_tarihi >= ?
        ");
        $sql->execute([$personel_id, $donem_bitis, $donem_baslangic]);
        $kayitlar = $sql->fetchAll(PDO::FETCH_OBJ);

        $toplamGun = 0;
        $donemBas = new \DateTime($donem_baslangic);
        $donemBit = new \DateTime($donem_bitis);

        foreach ($kayitlar as $k) {
            $bas = new \DateTime($k->baslangic_tarihi);
            $bit = new \DateTime($k->bitis_tarihi);
            $kesisimBas = max($bas, $donemBas);
            $kesisimBit = min($bit, $donemBit);
            if ($kesisimBas <= $kesisimBit) {
                $toplamGun += $kesisimBas->diff($kesisimBit)->days + 1;
            }
        }
        return $toplamGun;
    }
    ...
    public function getUcretsizIzinGunuDirekt($personel_id, $donem_baslangic, $donem_bitis)
    {
        ...
        foreach ($izinler as $izin) {
            ...
            for ($ts = $basTs; $ts !== false && $bitTs !== false && $ts <= $bitTs; $ts = strtotime('+1 day', $ts)) {
                $tarih = date('Y-m-d', $ts);
                if ($aktifGunler === null || isset($aktifGunler[$tarih])) {
                    $izinGunleri[$tarih] = true;
                }
            }
        }

        return count($izinGunleri);
    }
    ...
    public function getCalismaGunuSayisi($personel_id, $baslangic_tarihi, $bitis_tarihi)
    {
        ...
        // 2. Puantaj (personel_izinleri) 'X' veya RTÇ kodlu kayıtlardan gelen tarihler
        $sqlX = $this->db->prepare("
            ...
            AND pi.onay_durumu = 'Onaylandı'
            AND pi.silinme_tarihi IS NULL
            AND (LOWER(t.kisa_kod) = 'x' OR t.resmi_tatil_calismasi = 1)
            AND pi.baslangic_tarihi <= ?
            AND pi.bitis_tarihi >= ?
        ");
...

    {
        ...
        if (count($gecmisKayitlar) > 0) {
            ...
            foreach ($gecmisKayitlar as $idx => $g) {
                ...
                if ($hesapBitis >= $hesapBaslangic) {
                    ...
                    $asgariNetVal = floatval($this->genelAyarlarCache['asgari_ucret_net'] ?? 28075.50);
                    $asgariBrutVal = floatval($this->genelAyarlarCache['asgari_ucret_brut'] ?? 33030.00);
                    $sozlesmeTutari = floatval($g->maas_tutari);
                    if ($sozlesmeTutari <= 0) {
                        if ($isNetMaas) {
                            $sozlesmeTutari = $asgariNetVal;
                        } elseif (!$isPrimUsulu) {
                            $sozlesmeTutari = $asgariBrutVal;
                        }
                    }

                    if ($this->hasMaasaDahilSosyalYardim($kayit)) {
                        $gunlukAsgari = $asgariNetVal / 30;
                        $gunlukSozlesme = $sozlesmeTutari / 30;
            ...
            $nominalBrutMaas = floatval($kayit->maas_tutari ?? 0);
            if ($nominalBrutMaas <= 0) {
                if ($isNetMaas) {
                    $nominalBrutMaas = floatval($genelAyarlarMap['asgari_ucret_net'] ?? 28075.50);
                } elseif (!$isPrimUsulu) {
                    $nominalBrutMaas = floatval($genelAyarlarMap['asgari_ucret_brut'] ?? 33030.00);
                }
            }
        ...
        // Net maaş veya Brüt ise ve 0 ise asgari ücret kullan
        if ($brutMaas <= 0) {
            if ($isNetMaas) {
                $brutMaas = floatval($genelAyarlarMap['asgari_ucret_net'] ?? 28075.50);
            } elseif (!$isPrimUsulu) {
                $brutMaas = floatval($genelAyarlarMap['asgari_ucret_brut'] ?? 33030.00);
            }
        }
        ...
        $this->olusturProfilBazliOdemeler($kayit->personel_id, $kayit->donem_id, $kayit->baslangic_tarihi, $maasHesapGunu);
        ...
        if ($bazAlinacakTutar <= 0) {
            if ($isNetMaas) {
                $bazAlinacakTutar = floatval($genelAyarlarMap['asgari_ucret_net'] ?? 28075.50);
            } elseif (!$isPrimUsulu) {
                $bazAlinacakTutar = floatval($genelAyarlarMap['asgari_ucret_brut'] ?? 33030.00);
            }
        }
        
        // Maaşa Dahil kontrolü (Hakedişi asgari ücrete sabitleyip üzerini yemekle tamamlayacağız)
        if ($this->hasMaasaDahilSosyalYardim($kayit)) {
             $kayit->hedef_net_maas_tutari = ($nominalBrutMaas > 0) ? $nominalBrutMaas : floatval($genelAyarlarMap['asgari_ucret_net'] ?? 28075.50); 
             $bazAlinacakTutar = floatval($genelAyarlarMap['asgari_ucret_net'] ?? 28075.50);
        ...
        if ($karisikMaasOzeti !== null) {
            // Prim usulü parçada aylık maaş oluşmaz; o parçanın kazancı puantaj ek ödemelerinden gelir.
            // Sabit maaş parçası ise yalnızca kendi tarih aralığı ve ücretli günleri kadar hak edilir.
            $brutMaas = floatval($karisikMaasOzeti['sabit_hakedis']);
            $ucretsizIzinDusumu = 0;
            $fiiliCalismaGunuTemp = $maasHesapGunu;
        } elseif ($isNetMaas || $maasDurumu === 'brüt') {
        ...
        foreach ($ekOdemeler as $odeme) {
            $tutar = floatval($odeme->tutar);
            $resmiDahilEkToplam += floatval($odeme->resmi_tutar ?? 0);
            ...
            if ($isPuantajOdeme) {
                ...
                if ($isPrimUsuluDahilYardim) {
                    ...
                    // Net maaşlı personelde puantaj, toplam hakedişe eklenir; resmî
                    // banka tavanını büyütmez ve bu nedenle elden dağıtılır.
                    if ($isNetMaas) {
                        $yontem = 'elden';
                    }
        ...
        if ($rtcGunHesap > 0 || $htcGunHesap > 0) {
            ...
            if ($rtcGunHesap > 0) {
                ...
                // RTÇ günü için garanti edilen "ele geçen" hedef: asgari ücretin günlük neti (gross-up bazı)
                $rtcHedefNetHesap = round($gunlukAsgariHesap * $rtcGunHesap, 2);
            }
        }
        if ($htcGunHesap > 0) {
        ...
        if ($rtcHtcBrutToplam > 0 && ($isNetMaas || $isPrimUsulu)) {
            ...
            $isHtcNetHedefMi = $this->hasMaasaDahilSosyalYardim($kayit);
            $rtcHtcSonuc = $this->hesaplaRtcHtcKesinti(
                $rtcHedefNetHesap,
                $isHtcNetHedefMi ? $htcResmiHedefNetHesap : $htcVergiBazli,
                $isHtcNetHedefMi,
                $rtcSgkDahil, $rtcGvDahil, $rtcDamgaDahil,
                $htcSgkDahil, $htcGvDahil, $htcDamgaDahil,
                $rtcHtcSgkOrani, $rtcHtcIssizlikOrani, $rtcHtcDamgaOrani,
                $kumulatifMatrah + $gelirVergisiMatrahi,
                $donemYil
            );

            $rtcHtcSgkIsci = $rtcHtcSonuc['toplam_sgk'];
            $rtcHtcIssizlikIsci = $rtcHtcSonuc['toplam_issizlik'];
            $rtcHtcDamgaVergisi = $rtcHtcSonuc['toplam_damga'];
            $rtcHtcGelirVergisi = $rtcHtcSonuc['toplam_gv'];
            $rtcHtcMatrahKatkisi = $rtcHtcSonuc['toplam_matrah_katkisi'];
            $rtcHtcBrutToplamGercek = $rtcHtcSonuc['toplam_brut'];
            $rtcHtcNetOdeme = $rtcHtcSonuc['toplam_net'];
            $rtcHtcKesintiToplam = round($rtcHtcBrutToplamGercek - $rtcHtcNetOdeme, 2);
            ...
            $rtcHtcBankaNetiHesap = round(floatval($rtcHtcSonuc['rtc']['net']) + floatval($rtcHtcSonuc['htc']['net']), 2);
        ...
        $damgaVergisiMatrahi = $calisanBrutMaas + $brutEkOdemeler + $damgaMatrahEkleri;
        if ($isNetMaas || $isPrimUsulu) {
            $rtcHtcDamgaMatrahKatkisi = ($rtcDamgaDahil ? ($rtcHtcSonuc['rtc']['brut'] ?? 0.0) : 0.0) + ($htcDamgaDahil ? ($rtcHtcSonuc['htc']['brut'] ?? 0.0) : 0.0);
            $damgaVergisiMatrahi += $rtcHtcDamgaMatrahKatkisi;
            $damgaVergisi = $rtcHtcDamgaVergisi + ($damgaMatrahEkleri * $actualDamgaVergisiOrani);
        } else {
        ...
        // Resolve SGK firm proration
        $eksikGunTarihleri = $this->getMaasEksikGunTarihleri((int) $kayit->personel_id, $donemTarihi, $donemBitis);
        $dagilim = $this->getSgkFirmaDagilimi(
            $kayit->personel_id,
            $donemTarihi,
            $donemBitis,
            $kayit->sgk_yapilan_firma ?? 'Yok',
            $kayit->ise_giris_tarihi ?? null,
            $kayit->isten_cikis_tarihi ?? null,
            $eksikGunTarihleri
        );
        $nonKurRatio = $dagilim['non_kur_ratio'];
        $nonKurUcretGunu = min($maasHesapGunu, intval($dagilim['payable_non_kur_days'] ?? round($maasHesapGunu * $nonKurRatio)));
        ...
        if ($maasaDahilYardimAktif) {
            $asgariYatacak = round(($asgariNetNominal / 30) * $nonKurUcretGunu, 2);
            ...
            $hesaplamaDetay['ozet']['yuvarlama_farki'] = $yuvarlamaFarki;
            ...
            if ($toplamDahilYardim > 0 && $isPrimUsuluDahilYardim) {
                ...
                $ekOdemeDetaylari[] = [
                    ...
                    'aciklama' => 'Maaşa Dahil Dengeleme',
        ...
        $netMaas = floatval($netMaas ?? $hakedisNetBeforeKesinti ?? 0);
        $bankaOdemesi = floatval($bankaOdemesi ?? 0);
        ...
        $hesaplamaDetay = [
            ...
            'ozet' => [
                ...
                'damga_matrah_ekleri' => round($damgaMatrahEkleri, 2),
        ...
        return $this->saveBordroHesaplama($bordro_personel_id, [
            ...
            'toplam_kesinti' => round(($toplamKesinti - $icraKesintisi), 2), 'toplam_ek_odeme' => round($toplamEkOdeme, 2), 'fazla_mesai_tutar' => round($toplamMesaiTutar, 2),
