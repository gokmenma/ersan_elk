            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function initMacControls() {
                    ...
                    $('#dashboard-widgets .widget-item').each(function() {
                        ...
                        if (card.length && card.find('.mac-title-bar').length === 0) {
                            ...
                            controls.find('.mac-close').on('click mousedown touchstart', function (e) {
                                e.preventDefault();
                                e.stopPropagation();
                                const widget = $(this).closest('.widget-item');
                            ...
                            controls.find('.mac-minimize').on('click mousedown touchstart', function (e) {
                                e.preventDefault();
                                e.stopPropagation();
                                const widget = $(this).closest('.widget-item');
                            ...
                            controls.find('.mac-maximize').on('click mousedown touchstart', function (e) {
                                e.preventDefault();
                                e.stopPropagation();
                                const widget = $(this).closest('.widget-item');
                ...
                $(document).on('click mousedown touchstart', '.mac-close', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    const widget = $(this).closest('.widget-item');
                    const widgetId = widget.attr('id');
                ...
                $(document).on('click', '.mac-minimize', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    const widget = $(this).closest('.widget-item');
                    const cardBody = widget.find('.card-body');
                ...
                $(document).on('click', '.mac-maximize', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    const widget = $(this).closest('.widget-item');
                    maxZIndex++;
                ...
                function appendResizeHandles() {
                    const smallCards = [
                        ...
                        'widget-gec-kalanlar',
                        'widget-nobetciler',
                        'widget-gunluk-muhurleme',
                        'widget-gunluk-kesme-acma',
                        'widget-gunluk-endeks-okuma',
                        'widget-gunluk-sayac-degisimi',
                        'widget-kacak-sayisi'
                    ];

                    $("#dashboard-widgets .widget-item").each(function () {
                        const id = $(this).attr('id');
                        if (!id || smallCards.includes(id) || id === 'widget-row-break') return;

                        $(this).addClass('resizable-widget');
                        let card = $(this).find('.card');
                        if (card.length === 0) {
                            card = $(this).find('.carousel').first();
                        }
                        if (card.length && card.find('.custom-resize-handle').length === 0) {
                            card.css('overflow', 'visible').append('<div class="custom-resize-handle" title="Boyutlandır"></div>');
                        }
                    });
                }
                ...
                // Custom Resizing via Event Delegation
                $(document).on('mousedown touchstart', '.custom-resize-handle', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    let isResizing = true;
                    const widget = $(this).closest('.widget-item');
                    ...
                    let startX = e.type.startsWith('touch') ? e.originalEvent.touches[0].clientX : e.clientX;
                    let startY = e.type.startsWith('touch') ? e.originalEvent.touches[0].clientY : e.clientY;
                    let startWidth = widget.outerWidth();
                    let startHeight = widget.outerHeight();

                    $(document).on('mousemove.widgetResize touchmove.widgetResize', function (e) {
                        ...
                        if (newHeight >= 100) {
                            ...
                            widget.find('.card, .carousel, .carousel-inner, .card-body').css({
                                'height': '100%',
                                'min-height': '0',
                                'max-height': 'none'
                            });
                    ...
                    $(document).on('mouseup.widgetResize touchend.widgetResize', function () {
                        if (isResizing) {
                            isResizing = false;
